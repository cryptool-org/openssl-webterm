...
class GenKeysTab extends React.Component {
  ...
  render() {
    ...
    if (
      this.state.pubkeyFields.inputprivkey != '' && // "" means "Select file"
    ...
    if (this.state.pubkeyFields.inputprivkey != '') {
      let inputprivkeyFile = this.props.files.find(
        (file) => file.name == this.state.pubkeyFields.inputprivkey
      );
      this.isInputprivkeyEncrypted = inputprivkeyFile
        ? Helpers.isKeyEncrypted(inputprivkeyFile)
        : undefined;
    ...
    return (
      <>
        <Form.Group>
          <Form.Label className="d-inline-block mr-3 mb-0">
          ...
          <Form.Check
            ...
            name="keytype"
            value="ec"
            label={i18next.t('Elliptic curves')}
            onChange={(e) => this.onChange(e)}
            checked={this.state.keytype == 'ec'}
            id="genkey-keytype-ec"
          />
        </Form.Group>
          ...
          <Card className="mt-4">
            <Card.Header className={'d-flex align-items-center justify-content-between'}>
              <b>
                1){' '}
                {this.state.keytype != 'ec'
                  ? i18next.t('Generate private key')
                  : i18next.t('Generate parameters')}
              </b>
              ...
              <Row>
                <Col xs={12} md={this.state.keytype == 'ec' ? 12 : 6}>
                  {this.state.keytype == 'rsa' && (
                    <Form.Group>
                      ...
                      <Form.Control
                        required
                        as="select"
                        value={this.state.privkeyFields.keylength}
                        name="keylength"
                        onChange={(e) => this._onPrivkeyFieldChange(e)}
                        isValid={whatsValidPriv.keylength}
                        isInvalid={this._isInvalid(whatsValidPriv.keylength)}
                      >
                        {this._numbits.map((numbit) => (
                          <option key={numbit} value={numbit}>
                            {numbit}-bit
                          </option>
                        ))}
                      </Form.Control>
                  ...
                  {this.state.keytype == 'ec' && (
                    <Form.Group>
                      <Form.Label>
                        <Trans>Elliptic curve name</Trans>
                      </Form.Label>
                      <Form.Control
                        required
                        as="select"
                        value={this.state.privkeyFields.elcurvename}
                        name="elcurvename"
                        onChange={(e) => this._onPrivkeyFieldChange(e)}
                        isValid={whatsValidPriv.elcurvename}
                        isInvalid={this._isInvalid(whatsValidPriv.elcurvename)}
                      >
                  ...
                  <Form.Group>
                    <Form.Label>
                      <Form.Check
                        ...
                        type="checkbox"
                        name="useoutputfile"
                        id="genkey-privkey-useoutputfile"
                        label={i18next.t('Output to file')}
                        value={this.state.privkeyFields.useoutputfile == 'true' ? 'false' : 'true'}
                        onChange={(e) => this._onPrivkeyFieldChange(e)}
                        checked={this.state.privkeyFields.useoutputfile == 'true'}
                      />
                    </Form.Label>
                    <Form.Control
                      type="text"
                      value={this.state.privkeyFields.outputfile || ''}
                      name="outputfile"
                      onChange={(e) => this._onPrivkeyFieldChange(e)}
                      disabled={this.state.privkeyFields.useoutputfile != 'true'}
                      isInvalid={this._isInvalid(whatsValidPriv.outputfile)}
                      isValid={whatsValidPriv.outputfile}
                    />
                  </Form.Group>
                </Col>
                ...
                {this.state.keytype == 'rsa' && (
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="d-block">
                        <Form.Check
                          ...
                          type="checkbox"
                          name="encrypt"
                          id="genkey-privkey-encrypt"
                          className="mr-3"
                          label={i18next.t('Encrypt')}
                          value={this.state.privkeyFields.encrypt == 'true' ? 'false' : 'true'}
                          onChange={(e) => this._onPrivkeyFieldChange(e)}
                          checked={this.state.privkeyFields.encrypt == 'true'}
                        />
                        <Form.Check
                          custom
                          inline
                          ...
                          label="Text"
                          onChange={(e) => this._onPrivkeyFieldChange(e)}
                          id="genkey-privkey-passphrasetype-text"
                        ...
                        <Form.Check
                          ...
                          onChange={(e) => this._onPrivkeyFieldChange(e)}
                          id="genkey-privkey-passphrasetype-file"
                          disabled={this.state.privkeyFields.encrypt != 'true'}
                      ...
                      {this.state.privkeyFields.passphrasetype == 'text' && (
                        <Form.Control
                          type="password"
                          placeholder={
                            this.state.privkeyFields.encrypt == 'true'
                              ? i18next.t('Enter passphrase ..')
                              : ''
                          }
                          name="passphrasetext"
                          value={
                            this.state.privkeyFields.encrypt == 'true'
                              ? this.state.privkeyFields.passphrasetext || ''
                              : ''
                          }
                          onChange={(e) => this._onPrivkeyFieldChange(e)}
                          disabled={this.state.privkeyFields.encrypt != 'true'}
                          isInvalid={this._isInvalid(whatsValidPriv.passphrasetext)}
                          isValid={whatsValidPriv.passphrasetext}
                        />
                      )}
                      ...
                      {this.state.privkeyFields.passphrasetype == 'file' && (
                        <Form.Control
                          ...
                          value={
                            this.state.privkeyFields.encrypt == 'true'
                              ? this.state.passphrasefile
                              : ''
                          }
                          name="passphrasefile"
                          onChange={(e) => this._onPrivkeyFieldChange(e)}
                          disabled={this.state.privkeyFields.encrypt != 'true'}
                          isInvalid={this._isInvalid(whatsValidPriv.passphrasefile)}
                          isValid={whatsValidPriv.passphrasefile}
                        >
                          <option key={0} value="">
                            {this.state.privkeyFields.encrypt == 'true'
                    ...
                    <Form.Group>
                      <Form.Label
                        className={this.state.privkeyFields.encrypt != 'true' ? 'text-muted' : ''}
                      >
                        <Trans>Cipher (for encryption)</Trans>
                      </Form.Label>
                      <Form.Control
                        as="select"
                        value={
                          this.state.privkeyFields.encrypt == 'true'
                            ? this.state.privkeyFields.cipher
                            : ''
                        }
                        name="cipher"
                        onChange={(e) => this._onPrivkeyFieldChange(e)}
                        disabled={this.state.privkeyFields.encrypt != 'true'}
                        isInvalid={this._isInvalid(whatsValidPriv.cipher)}
                        isValid={whatsValidPriv.cipher}
                      >
                        <option key={0} value="">
                          {this.state.privkeyFields.encrypt == 'true'
                            ? i18next.t('Select cipher')
                            : ''}
                        </option>
                        {this.props.cipherList.map((cipher) => (
        ...
        <Form onSubmit={(e) => e.preventDefault()}>
          <Card className="mt-4">
            <Card.Header className={'d-flex align-items-center justify-content-between'}>
              <b>
                2) <Trans>Derive public key</Trans>
              </b>
              ...
              <Row>
                <Col xs={12} md={this.state.keytype == 'ec' ? 4 : 6}>
                  <Form.Group>
                    <Form.Label>
                      <Trans>
                        {this.state.keytype != 'ec' ? 'Private key' : 'EC params'} input file
                      </Trans>
                    </Form.Label>
                    <Form.Control
                      required
                      as="select"
                      value={this.state.pubkeyFields.inputprivkey}
                      name="inputprivkey"
                      onChange={(e) => this._onPubkeyFieldChange(e)}
                      isValid={whatsValidPub.inputprivkey}
                      isInvalid={this._isInvalid(whatsValidPub.inputprivkey)}
                    >
                      <option key={0} value="">
                        {i18next.t('Select file')}
                      </option>
                ...
                {this.state.keytype == 'rsa' && (
                  <Col xs={12} md={6}>
                    <Form.Group>
                      <Form.Label className="d-block">
                        <span
                          className={'mr-3 ' + (!this.isInputprivkeyEncrypted ? 'text-muted' : '')}
                        >
                          <Trans>Decrypt private key</Trans>:
                        </span>
                        <Form.Check
                          custom
                          inline
                          ...
                          label="Text"
                          onChange={(e) => this._onPubkeyFieldChange(e)}
                          id="genkey-pubkey-passphrasetype-text"
                          disabled={!this.isInputprivkeyEncrypted}
                          checked={this.state.pubkeyFields.passphrasetype == 'text'}
                        />
                        <Form.Check
                          custom
                          inline
                          ...
                          label={i18next.t('File')}
                          onChange={(e) => this._onPubkeyFieldChange(e)}
                          id="genkey-pubkey-passphrasetype-file"
                          disabled={!this.isInputprivkeyEncrypted}
                          checked={this.state.pubkeyFields.passphrasetype == 'file'}
                        />
                      </Form.Label>
                      ...
                      {this.state.pubkeyFields.passphrasetype == 'text' && (
                        <Form.Control
                          type="password"
                          placeholder={
                            this.isInputprivkeyEncrypted
                              ? i18next.t('Enter passphrase ..')
                              : this.state.pubkeyFields.inputprivkey?.length > 0
                              ? i18next.t('Private key not encrypted')
                              : i18next.t('No private key selected')
                          }
                          name="passphrasetext"
                          value={
                            this.isInputprivkeyEncrypted
                              ? this.state.pubkeyFields.passphrasetext || ''
                              : ''
                          }
                          onChange={(e) => this._onPubkeyFieldChange(e)}
                          disabled={!this.isInputprivkeyEncrypted}
                          isInvalid={this._isInvalid(whatsValidPub.passphrasetext)}
                          isValid={whatsValidPub.passphrasetext}
                        />
                      )}
                      ...
                      {this.state.pubkeyFields.passphrasetype == 'file' && (
                        <Form.Control
                          ...
                          value={
                            this.isInputprivkeyEncrypted
                              ? this.state.pubkeyFields.passphrasefile
                              : ''
                          }
                          name="passphrasefile"
                          onChange={(e) => this._onPubkeyFieldChange(e)}
                          disabled={!this.isInputprivkeyEncrypted}
                          isInvalid={this._isInvalid(whatsValidPub.passphrasefile)}
                          isValid={whatsValidPub.passphrasefile}
                        >
                          <option key={0} value="">
                            {i18next.t(
                              this.isInputprivkeyEncrypted
                  ...
                  <Form.Group>
                    <Form.Label>
                      <Form.Check
                        ...
                        type="checkbox"
                        name="useoutputfile"
                        id="genkey-pubkey-useoutputfile"
                        label={i18next.t('Output to file')}
                        value={this.state.pubkeyFields.useoutputfile == 'true' ? 'false' : 'true'}
                        onChange={(e) => this._onPubkeyFieldChange(e)}
                        checked={this.state.pubkeyFields.useoutputfile == 'true'}
                      />
                    </Form.Label>
                    <Form.Control
                      type="text"
                      value={this.state.pubkeyFields.outputfile}
                      name="outputfile"
                      onChange={(e) => this._onPubkeyFieldChange(e)}
                      disabled={this.state.pubkeyFields.useoutputfile != 'true'}
                      isInvalid={this._isInvalid(whatsValidPub.outputfile)}
                      isValid={whatsValidPub.outputfile}
                    />
                  </Form.Group>
                </Col>
                ...
                <Col xs={12} md={this.state.keytype == 'ec' ? 4 : 6}>
                  <Form.Group>
                    <Form.Label>
                      <Trans>Public key output format</Trans>
                    </Form.Label>
                    <Form.Control
                      required
                      as="select"
                      value={this.state.pubkeyFields.keyformat}
                      name="keyformat"
                      onChange={(e) => this._onPubkeyFieldChange(e)}
                      isValid={whatsValidPub.keyformat}
                      isInvalid={this._isInvalid(whatsValidPub.keyformat)}
                    >
                      {this._keyFormats.map((format) => (
  ...
  onChange(e) {
    ...
    if (e.target.name == 'keytype')
      this.state.pubkeyFields.inputprivkey = this._initialState.pubkeyFields.inputprivkey; // todo: maybe copy not reference?
  ...
  _onPrivkeyFieldChange(e) {
    this.setState({
      privkeyFields: { ...this.state.privkeyFields, [e.target.name]: e.target.value },
    });
  }
  ...
  _onPubkeyFieldChange(e) {
    let pubkeyFields = { ...this.state.pubkeyFields, [e.target.name]: e.target.value };

    // special case: replace extension in outputfile on keyformat change
    if (e.target.name == 'keyformat')
      pubkeyFields.outputfile = pubkeyFields.outputfile.replace(
        new RegExp('\\.(' + this.state.pubkeyFields.keyformat.toLowerCase() + ')$'),
