...
class EncryptionTab extends React.Component {
  // should get props: files, cipherList, runCommand
  ...
  render() {
    ...
    if (this.state.key != 0 && !keyfiles.includes(this.state.key)) this.state.key = undefined;

    // set default key file (on files update)
    if (this.state.key == undefined && keyfiles.length > 0) this.state.key = keyfiles[0];
    ...
    return (
      <Form onSubmit={(e) => e.preventDefault()}>
        <Row className="flex-md-row-reverse justify-content-between">
          ...
          <Col xs={12} md="auto">
            <Form.Group>
              <Form.Label className="d-inline-block mr-3">
                <Trans>Mode</Trans>:
              </Form.Label>
              <Form.Check
                custom
              ...
              <Form.Check
                ...
                name="mode"
                value="decrypt"
                label={i18next.t('Decrypt')}
                onChange={(e) => this.onChange(e)}
                id="encdec-mode-decrypt"
            ...
            <Form.Group>
              <Form.Label>
                ...
                <Form.Check
                  ...
                  name="method"
                  value="cipher"
                  label={i18next.t('Cipher')}
                  onChange={(e) => this.onChange(e)}
                  id="encdec-method-cipher"
                ...
                <Form.Check
                  ...
                  value="key"
                  label={
                    (this.state.mode == 'encrypt' ? 'Public' : 'Private') + ' ' + i18next.t('key')
                  }
                  onChange={(e) => this.onChange(e)}
                  id="encdec-method-key"
              ...
              {this.state.method == 'cipher' && (
                <Form.Control
                  ...
                  name="cipher"
                  onChange={(e) => this.onChange(e)}
                  isInvalid={this._isInvalid(whatsValid.cipher)}
                  isValid={whatsValid.cipher}
                >
              ...
              {this.state.method == 'key' && (
                <Form.Control
                  ...
                  name="key"
                  onChange={(e) => this.onChange(e)}
                  isInvalid={this._isInvalid(whatsValid.key)}
                  isValid={whatsValid.key}
                >
                  <option key={0} value="">
                    {i18next.t('Select file')}
                  </option>
                  {keyfiles.map((keyfile) => (
                    <option key={keyfile} value={keyfile}>
                      {keyfile}
                    </option>
                  ))}
                </Form.Control>
          ...
          <Col xs={12} md={6}>
            <Form.Group>
              <Form.Label
                className={
                  'd-block ' +
                  ((this.state.method == 'key' && this.state.mode == 'encrypt') ||
                  (this.state.method == 'key' &&
              ...
              >
                <span className="mr-3">{i18next.t('Passphrase')}:</span>
                <Form.Check
                  custom
                  inline
                  type="radio"
              ...
              {this.state.passphrasetype == 'text' && (
                <Form.Control
                  ...
                  placeholder={
                    this.state.method == 'key' && this.state.mode == 'encrypt'
                      ? i18next.t('Public keys are not encrypted')
                      : this.state.method == 'key' &&
                        this.state.mode == 'decrypt' &&
                        !this.isPrivateKeyEncrypted
                  ...
                  isInvalid={this._isInvalid(whatsValid.passphrasetext)}
                  isValid={whatsValid.passphrasetext}
                />
              )}
              ...
              {this.state.passphrasetype == 'file' && (
                <Form.Control
                  ...
                  name="passphrasefile"
                  onChange={(e) => this.onChange(e)}
                  isInvalid={this._isInvalid(whatsValid.passphrasefile)}
                  isValid={whatsValid.passphrasefile}
                  disabled={
                    (this.state.method == 'key' && this.state.mode == 'encrypt') ||
                    (this.state.method == 'key' &&
                      this.state.mode == 'decrypt' &&
        ...
        <Row>
          ...
          <Col xs={12} md={6}>
            ...
            <Form.Check
              ...
              id="encdec-pbkdf2"
              label="PBKDF2"
              value={this.state.pbkdf2 == 'true' ? 'false' : 'true'}
              onChange={(e) => this.onChange(e)}
              checked={this.state.method == 'key' ? false : this.state.pbkdf2 == 'true'}
            ...
            <Form.Check
              ...
              id="encdec-salt"
              label="Salt"
              value={this.state.salt == 'true' ? 'false' : 'true'}
              onChange={(e) => this.onChange(e)}
              checked={this.state.method == 'key' ? false : this.state.salt == 'true'}
              disabled={this.state.method == 'key'}
            />
            <Form.Check
              custom
              ...
              id="encdec-base64"
              label="Base64"
              value={this.state.base64 == 'true' ? 'false' : 'true'}
              onChange={(e) => this.onChange(e)}
              checked={this.state.method == 'key' ? false : this.state.base64 == 'true'}
              disabled={this.state.method == 'key'}
            />
          </Col>
        </Row>
  ...
  _validateFields() {
    ...
    if (this.state.inputtype == 'text') whatsValid.inputtext = !!this.state.inputtext;

    // check if input file is valid
    if (this.state.inputtype == 'file') whatsValid.inputfile = !!this.state.inputfile;
    ...
    if (this.state.method == 'key') whatsValid.key = !!this.state.key;

    // only use passphrase for ciphers and decryption with encrypted private keys
    if (
      this.state.method == 'cipher' ||
      (this.state.mode == 'decrypt' && this.isPrivateKeyEncrypted)
    ) {
  ...
  _buildCommand(whatsValid = {}) {
    ...
    if (this.state.inputtype == 'file') command += ' -in ' + this.state.inputfile;

    if (this.state.method == 'cipher') {
      if (this.state.passphrasetype == 'text') command += ' -k ' + this.state.passphrasetext;

      if (this.state.passphrasetype == 'file') command += ' -kfile ' + this.state.passphrasefile;

      if (this.state.pbkdf2 == 'true') command += ' -pbkdf2';

      if (this.state.salt != 'true') command += ' -nosalt';

      if (this.state.base64 == 'true') command += ' -a';
    }
